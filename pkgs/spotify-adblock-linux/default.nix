...
let
  ...
  stdenv.mkDerivation rec {
    ...
    unpackPhase = ''
      # extract 3 MB files from 300 MB archive takes ~1 minute
      echo unpacking source from ${src-tar}
      mkdir source
      tar -C source/ -xf ${src-tar} --wildcards '*/include' --strip-components=1
    '';
...
stdenv.mkDerivation rec {
  ...
  buildInputs = [
    glib # util fns. also required by spotify -> zero cost
  ...
  src = fetchFromGitHub {
    ...
    # fork
    owner = "milahu";
    rev = "7db63b4f4442253523e099c8acd451186b735387";
    sha256 = "0zxq4spfy7k21szwffl4mkhgl211bqyngc40qqwv7j6grd5nfnmp";

    name = "source"; # TODO verify
  };
  #*/

  # DEVEL use local version of spotify-adblock-linux
  /*
  src = /home/user/src/spotify-adblock-linux;
  unpackPhase = ''
    runHook preUnpack

    mkdir -p ${sourceRoot}
    cp -r $src/* ${sourceRoot}/
    chown -R nixbld:nixbld ${sourceRoot}
  ...
  preBuild = ''
    ...
    ln -s -v ${cef-binary-include}/include ./
  '';
  ...
  meta = with lib; {
    ...
    longDescription = ''
      ...
      ### spotify hangs on start

      a brutal solution is to remove spotify's cache files:

      ```bash
      rm -rf ~/.cache/spotify/

      # or:
      rm -rf ~/.cache/spotify/{Browser,Data}
      ```

      ### GPU process isn't usable. Goodbye.

      sometimes we just need to retry starting spotify multiple times ... until it works
